...
- When live MCP sessions lock `obsidian-mcp.exe` on this machine, local validation may use `uv run --no-sync ...`; CI runners are unaffected. Do not terminate live Codex sessions' MCP processes without approval.
